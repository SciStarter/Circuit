...
fn interpret_one<Tz: TimeZone>(
    ...
    opp.exterior.opp.uid = uuid::Uuid::new_v5(&partner.partner, data.global_id.as_bytes());
    ...
    opp.exterior.opp.partner_created = match data.date_utc.split_once(' ') {
    ...
    opp.exterior.opp.partner_updated = match data.modified_utc.split_once(' ') {
        Some(pair) => match DateTime::parse_from_rfc3339(&format!("{}T{}Z", pair.0, pair.1)) {
            Ok(dt) => Some(dt),
            Err(_) => None,
        },
    ...
    opp.exterior.opp.partner_opp_url = Some(data.url);

    opp.exterior.opp.organization_name = partner.partner_name.clone();

    opp.exterior.opp.organization_website = partner.partner_website.clone();
    ...
    opp.exterior.opp.title = htmlentity::entity::decode(data.title.as_bytes())
        .to_string()
        .unwrap_or_default();

    opp.exterior.opp.description = common::html_to_md(&data.description);

    if let Optional::Value(img) = data.image {
        opp.exterior.opp.image_url = img.url;
    }
    ...
    opp.exterior.opp.cost = if partner.flags.contains(&crate::structure::PartnerFlag::Cost) {
    ...
    if let Some(venue) = data.venue {
        ...
        opp.exterior.opp.location_name = venue.name.clone();
        opp.exterior.opp.address_street = venue.address.clone();
        opp.exterior.opp.address_city = venue.city.clone();
        opp.exterior.opp.address_state = venue.state.clone();
        opp.exterior.opp.address_zip = venue.zip.clone();
        opp.exterior.opp.address_country = venue.country.clone();
    ...
    opp.exterior.opp.ticket_required = partner
        .flags
        .contains(&crate::structure::PartnerFlag::Ticketed);

    opp.exterior.opp.organization_type = Default::default();
